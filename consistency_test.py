...
class TestConsistency(Tester):
    ...
    def test_tbd_transient(self):
        ...
        # the data should still be there for years 2011-2017, but prior to CASSANDRA-TBD it would've been gone
        assert_all(session,
                   "SELECT COUNT(*) FROM journals.logs WHERE user = 'beobal' AND year < 2018 ORDER BY year DESC;",
                   [[7 * 12 * 30]],
                   cl=ConsistencyLevel.ONE)
